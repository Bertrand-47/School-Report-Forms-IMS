<?php
  ...
  && isset($_POST['classID'])){
    $sql = mysqli_query($connect, "SELECT DISTINCT courses.`id`, `coursename`, `maxcat`, `maxexam`, `p_percentage`, sm.cat, sm.exam FROM `courses` LEFT JOIN student_marks sm ON sm.course = courses.id and sm.student= '{$_POST['studentID']}'
    WHERE sm.session = '{$_POST['sessionID']}'
    AND sm.term = '{$_POST['termID']}' AND sm.student = '{$_POST['studentID']}'
    UNION ALL SELECT courses.`id`, `coursename`, `maxcat`, `maxexam`, `p_percentage`, '','' FROM `courses` WHERE courses.session = '{$_POST['sessionID']}' and courses.id NOT IN (SELECT courses.`id` FROM `courses` LEFT JOIN student_marks sm
